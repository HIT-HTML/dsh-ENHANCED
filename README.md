...
- ⚠️ **Configuration required for `web_search`** — the plugin does not auto-configure the harness `searchProvider`; set the `free-search:` namespace (`provider: ddg` / `searxng` / etc.) in your settings and the `web-search-free` patch row in `cordis.patch.yml`. `web_fetch` needs no such setup.
