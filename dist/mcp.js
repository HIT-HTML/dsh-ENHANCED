...
import { MCP_BEGIN, MCP_END, assertName, patchPath, readPatch, splitBlock, splitInner } from "./shared.js";
export const MCP_ACTIONS = ["list_mcps", "mcp_status", "add_mcp", "remove_mcp", "enable_mcp", "disable_mcp"];
function parseRows(text) {
    ...
    if (!Array.isArray(parsed))
        return [];
    if (parsed.length === 1 && parsed[0] && typeof parsed[0] === "object" && Array.isArray(parsed[0].insert)) {
        return parsed[0].insert;
    }
    return parsed;
}
async function writeRows(profile, rows) {
...
function findRow(rows, mcpName) {
    return rows.find((r) => r.config?.serverName === mcpName);
}
function toInfo(profile, row) {
    const c = row.config || {};
    const info = {
        profile,
...
export const handleMcp = async function (action, args, env) {
    ...
    switch (action) {
        ...
        case "mcp_status": {
            ...
            try {
                for (const n of ctx.tools.view().visible.keys())
                    toolNames.add(String(n));
            }
            catch { }
            const fibers = new Set();
            try {
                for (const entry of env.loaderRef?.entries?.() ?? []) {
                    if (entry && entry.fiber !== undefined)
                        fibers.add(String(entry.id ?? entry.name));
                }
            }
            catch { }
            const servers = [];
            for (const profile of profiles) {
                for (const row of parseRows(await readPatch(profile))) {
                    ...
                    if (row.disabled === true)
                        ...
                        if (state === "not loaded" && fibers.has(String(row.id ?? `mcp-${serverName}`)))
                            state = "connecting";
                    }
        ...
        case "disable_mcp": {
            ...
            if (!touched)
                return { error: `MCP server "${name}" not found in profiles: ${targets.join(", ")}` };
