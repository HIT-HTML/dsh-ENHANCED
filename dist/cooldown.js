...
export function classifyFailure(message) {
    ...
    if (QUOTA_PATTERNS.some((p) => p.test(s)))
        return { kind: "quota", ttlMs: QUOTA_COOLDOWN_MS };
...
export function loadCooldownState(path = COOLDOWN_STATE_PATH) {
    try {
...
export function saveCooldownState(state, path = COOLDOWN_STATE_PATH) {
    mkdirSync(dirname(path), { recursive: true, mode: 0o700 });
    const tmp = `${path}.${process.pid}.${Date.now()}.tmp`;
    writeFileSync(tmp, `${JSON.stringify(state, null, 2)}\n`, { mode: 0o600 });
    renameSync(tmp, path);
}
export function removeCooldownState(path = COOLDOWN_STATE_PATH) {
    try {
        unlinkSync(path);
    }
    catch (error) {
...
export function clearCooldown(state, engine) {
...
export function pruneCooldowns(state, now = Date.now()) {
    let removed = false;
    for (const [engine, entry] of Object.entries(state.engines || {})) {
        if (!entry || !(typeof entry.until === "string" && Date.parse(entry.until) > now)) {
            delete state.engines[engine];
