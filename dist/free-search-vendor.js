/**
 * Vendored Free Search engine — ported from dsh-free-search v0.4.12
 * (https://github.com/DDDMUC/dsh-free-search, MIT License, Copyright (c) DDDMUC).
 *
 * Copied wholesale so dsh-enhanced ships search without depending on a package
 * whose peerDependencies (@deepseek-ai/dsh-tools) are not published to npm —
 * fresh installs of the standalone plugin can fail outright. Upstream fixes do
 * NOT propagate automatically; re-port when adopting upstream changes.
 *
...
const FREE_SEARCH_NS = settingsNamespace("enhanced-free-search");
...
function buildChain(preferred, cfg, timeRange, cooling) {
    ...
    if (blocked(preferred))
        preferredSkippedReason = excluded.includes(preferred) ? "excluded" : "cooldown";
    else if (timeRange && !TIME_ENGINES.includes(preferred))
...
function parseTimeRange(input) {
    ...
    if (typeof input === "object") {
        if (typeof input.after === "string" && /^\d{4}-\d{2}-\d{2}$/.test(input.after))
...
async function searchGithub(query, maxResults, signal) {
    const response = await fetch(`https://api.github.com/search/repositories?q=${encodeURIComponent(query)}&per_page=${maxResults ?? 5}`, {
...
async function searchV2ex(query, maxResults, signal) {
    ...
    if (!response.ok)
        throw new Error(`V2EX API error (HTTP ${response.status})`);
...
async function searchBilibili(query, maxResults, signal) {
    const response = await fetch(`https://api.bilibili.com/x/web-interface/search/all/v2?keyword=${encodeURIComponent(query)}`, {
    ...
    for (const section of data.data?.result ?? []) {
        for (const item of section.data ?? []) {
            ...
            if (sources.length >= (maxResults ?? 5))
                break;
        }
        if (sources.length >= (maxResults ?? 5))
            break;
...
async function searchReddit(query, maxResults, signal) {
    const response = await fetch(`https://old.reddit.com/search.json?q=${encodeURIComponent(query)}&limit=${maxResults ?? 5}&sort=relevance`, {
        headers: {
...
async function searchHackerNews(query, maxResults, signal) {
    const response = await fetch(`https://hn.algolia.com/api/v1/search?query=${encodeURIComponent(query)}&hitsPerPage=${maxResults ?? 5}`, {
        headers: { "user-agent": USER_AGENT, accept: "application/json" },
        ...(signal !== undefined ? { signal } : {}),
    });
    if (!response.ok)
        throw new Error(`Hacker News API error (HTTP ${response.status})`);
    const data = await response.json();
    return {
        sources: (data.hits ?? [])
...
async function searchStackOverflow(query, maxResults, signal) {
    const response = await fetch(`https://api.stackexchange.com/2.3/search/advanced?order=desc&sort=relevance&q=${encodeURIComponent(query)}&site=stackoverflow&pagesize=${maxResults ?? 5}&filter=!nNPvSNVZJS`, {
        headers: { "user-agent": USER_AGENT, accept: "application/json" },
        ...(signal !== undefined ? { signal } : {}),
    });
    if (!response.ok)
        throw new Error(`Stack Exchange API error (HTTP ${response.status})`);
    const data = await response.json();
...
async function searchWikipedia(query, maxResults, signal, lang) {
    ...
    const response = await fetch(`https://${host}/w/api.php?action=query&list=search&srsearch=${encodeURIComponent(query)}&format=json&srlimit=${maxResults ?? 5}`, {
        headers: { "user-agent": USER_AGENT, accept: "application/json" },
        ...(signal !== undefined ? { signal } : {}),
    });
    if (!response.ok)
...
async function searchNpm(query, maxResults, signal) {
    const response = await fetch(`https://registry.npmjs.com/-/v1/search?text=${encodeURIComponent(query)}&size=${maxResults ?? 5}`, {
        headers: { "user-agent": USER_AGENT, accept: "application/json" },
        ...(signal !== undefined ? { signal } : {}),
    });
    if (!response.ok)
        throw new Error(`npm registry API error (HTTP ${response.status})`);
    const data = await response.json();
    return {
...
async function searchExa(query, maxResults, apiKey, timeRange, signal, baseUrl) {
    if (!apiKey)
        throw new Error("Exa search requires EXA_API_KEY");
    const body = {
        query,
        type: "auto",
    ...
    if (timeRange) {
        if (timeRange.after)
            body.startPublishedDate = timeRange.after;
        else if (timeRange.days !== undefined)
            body.startPublishedDate = isoDaysAgo(timeRange.days);
    }
    ...
    if (!response.ok) {
        ...
        if (response.status === 401) {
            throw new Error("Exa API key is invalid (HTTP 401) - update it in the dsh-enhanced plugin's Search section");
        }
...
async function searchTavily(query, maxResults, apiKey, timeRange, signal, baseUrl) {
    ...
    if (!response.ok) {
        const detail = await response.text().catch(() => "");
        if (response.status === 401) {
            throw new Error("Tavily API key is invalid (HTTP 401) - update it in the dsh-enhanced plugin's Search section");
        }
...
async function searchKeenableREST(query, maxResults, apiKey, timeRange, signal, baseUrl) {
    ...
    try {
        ...
        if (timeRange) {
            if (timeRange.after)
                body.published_after = timeRange.after;
            else if (timeRange.days !== undefined)
                body.published_after = formatKeenableRelative(timeRange.days);
        }
    ...
    if (!response.ok) {
        const detail = await response.text().catch(() => "");
        if (response.status === 401) {
            throw new Error("Keenable API key is invalid (HTTP 401) - update it in the dsh-enhanced plugin's Search section");
        }
...
async function searchKeenableMCP(query, maxResults, timeRange, signal) {
    ...
    try {
        ...
        if (timeRange) {
            if (timeRange.after)
                arguments_.published_after = timeRange.after;
            else if (timeRange.days !== undefined)
                arguments_.published_after = formatKeenableRelative(timeRange.days);
        }
...
async function searchKeenable(query, maxResults, apiKey, timeRange, signal, baseUrl) {
    if (apiKey)
        return searchKeenableREST(query, maxResults, apiKey, timeRange, signal, baseUrl);
    return searchKeenableMCP(query, maxResults, timeRange, signal);
}
async function searchPerplexity(query, maxResults, apiKey, signal) {
    if (!apiKey)
        throw new Error("Perplexity search requires PERPLEXITY_API_KEY");
    ...
    if (!response.ok) {
        const detail = await response.text().catch(() => "");
        if (response.status === 401) {
            throw new Error("Perplexity API key is invalid (HTTP 401) - update it in the dsh-enhanced plugin's Search section");
        }
...
async function searchDeepSeekOfficial(query, maxResults, apiKey, signal) {
    if (!apiKey)
        throw new Error("DeepSeek search requires DEEPSEEK_API_KEY");
    // 内置 20s 超时（与外部 signal 组合）：调用方不传 signal 时也不会永久卡住
    ...
    if (!response.ok) {
        const detail = await response.text().catch(() => "");
        if (response.status === 401) {
            throw new Error("DeepSeek API key is invalid (HTTP 401) - update it in the dsh-enhanced plugin's Search section");
        }
...
function apply(ctx, config) {
    ...
    const persistCooldown = (state) => {
        try {
            ...
            logger.warn(`free-search: cooldown state write failed (continuing without memory): ${error instanceof Error ? error.message : String(error)}`);
        }
    };
    ...
    const resolveApiKey = async (envName, settingsKey) => {
        const cfg = current();
        if (settingsKey && cfg[settingsKey])
            return cfg[settingsKey];
        if (credentials) {
            try {
    ...
    const provider = {
        ...
        async search(request, signal) {
            ...
            const preferred = typeof request.engine === "string" && ALL_ENGINES.includes(request.engine)
                ? request.engine
                : cfg.provider ?? "bing";
                ...
                try {
                    ...
                    if (engine === "ddg") {
                        ...
                        if (!key) {
                            ...
                            if (engine === preferred)
                                preferredFailure = "PERPLEXITY_API_KEY is not configured";
                        ...
                        if (!key) {
                            ...
                            if (engine === preferred)
                                preferredFailure = "DEEPSEEK_API_KEY is not configured";
                            logger.warn(`free-search: engine "${engine}" skipped (no key), trying next engine`);
                            continue; // 无 key 跳过
                        }
                    ...
                    if (result.sources.length > 0) {
                        ...
                        if (clearCooldown(coolState, engine))
                            persistCooldown(coolState);
                    ...
                    lastError = error;
                    const message = error instanceof Error ? error.message : String(error);
                    if (recordCooldown(coolState, engine, message))
                        persistCooldown(coolState);
                    if (engine === preferred)
    ...
    // (upstream installed its settings UI here; dsh-enhanced's Search tab owns that)
    ctx.inject(["webServer", "settings"], (sctx) => {
        sctx.effect(() => {
            const disposers = makeBridgeRoutes(sctx.settings, (request) => provider.search(request, undefined), (engine, query, timeRange) => runEngineTest(engine, query, timeRange)).map((route) => sctx.webServer.register(route));
            return () => {
    ...
    const runEngineTest = async (engine, query, timeRange) => {
        ...
        const attempt = async () => {
            switch (engine) {
                ...
                case "exa": {
                    const key = await resolveApiKey("EXA_API_KEY", "exaApiKey");
                    if (key)
                        return await searchExa(q, 2, key, tr, undefined, cfg.exaBaseUrl);
        ...
        try {
            ...
            return {
                ok: true,
                sources: (result.sources ?? []).map((s) => s.snippet ? { ...s, snippet: cleanSnippet(s.snippet) } : s),
    ...
    ctx.inject(["tools"], (sctx) => {
        sctx.effect(() => {
            const dispose = sctx.tools.register(defineTool({
                name: "free_search_test",
                description: "Test every configured web search engine and report which ones work. Use this to verify engine availability, diagnose search failures, or check whether an API key is configured.",
                parameters: {
                    engines: {
                        type: "array",
                ...
                async execute(args) {
                    ...
                    for (const engine of engines) {
                        ...
                        if (r.ok) {
                            ...
                            if (r.sources[0]?.title)
                                item.sampleTitle = String(r.sources[0].title);
                            if (r.sources[0]?.url)
                                item.sampleUrl = String(r.sources[0].url);
                            results.push(item);
                        }
                        else {
    ...
    ctx.inject(["tools"], (sctx) => {
        sctx.effect(() => {
            const dispose = sctx.tools.register(defineTool({
                ...
                description: "Search a specific platform (GitHub / V2EX / Bilibili / Reddit / Hacker News / Stack Overflow / Wikipedia / npm) for a query. Returns source URLs with titles and snippets. Use this when the user asks about repos, code, forum threads, videos, discussions, Q&A, encyclopedia entries, or packages.",
                parameters: {
                    platform: {
                        type: "string",
                ...
                async execute(args) {
                    ...
                    if (!enabled.includes(platform)) {
                        throw new Error(`platform "${platform}" is disabled in search settings - enable it in the dsh-enhanced plugin's Search section`);
                    }
                    ...
                    const sources = (result.sources ?? []).map((s) => {
                        ...
                        if (s.title !== undefined && s.title !== null && s.title !== "")
                            source.title = String(s.title);
                        if (s.snippet !== undefined && s.snippet !== null && s.snippet !== "")
                            source.snippet = String(s.snippet);
                        return source;
                    });
    ...
    ctx.inject(["tools"], (sctx) => {
        sctx.effect(() => {
            const dispose = sctx.tools.register(defineTool({
                ...
                description: "Search the web with optional time filtering. Use when the user wants results from a specific time window (e.g. 'last week', 'this month') or when you need to force a specific engine. Falls back across engines automatically just like web_search.",
                parameters: {
                    query: {
                        type: "string",
                        description: "The search query.",
                ...
                async execute(args) {
                    ...
                    if (parseTimeRange(args.timeRange) !== undefined)
                        request.timeRange = args.timeRange;
                    // engine 指定时：仅当该引擎可用才优先（仍走回退链，失败自动换引擎）
                    if (args.engine && ALL_ENGINES.includes(args.engine))
    ...
    ctx.inject(["systemPrompt"], (sctx) => {
        ...
        refreshPrompt = () => {
            ...
            disposeSection = sctx.systemPrompt.section({
                ...
                order: 500,
                text: [
                    "## Available web search engines (built into the dsh-enhanced plugin)",
                    "",
                    "You have the web_search tool. Its backend engine is chosen in the dsh-enhanced plugin's Search section.",
