...
import { SKILLS_DIR, assertName } from "./shared.js";
...
export const SKILL_ACTIONS = ["list_skills", "read_skill", "add_skill", "edit_skill", "remove_skill", "install_skill", "install_skill_files"];
 ...
 */
export function parseSkillDoc(raw) {
    const m = /^---\r?\n([\s\S]*?)\r?\n---/.exec(raw);
    if (!m)
        return {};
    try {
...
async function listManagedSkillDirs() {
    try {
...
async function writeSkill(name, description, body, whenToUse, extraFrontmatter) {
    const dir = join(SKILLS_DIR, name);
    await mkdir(dir, { recursive: true });
    let front = `---\nname: ${scalar(name)}\ndescription: ${scalar(description)}\n`;
    if (whenToUse)
        front += `whenToUse: ${scalar(whenToUse)}\n`;
...
async function findSkillBundles(root, depth = 6) {
    if (depth < 0)
        return [];
    const out = [];
    let entries;
...
async function installBundle(srcReal, nameHint, overwrite) {
    try {
...
async function installMany(bundles, displayRoot, overwrite) {
    const installed = [];
    const skipped = [];
    const failed = [];
    for (const bundle of bundles.sort()) {
        ...
        if (r.status === "ok")
            ...
            failed.push({ path: displayRoot ? bundle.slice(displayRoot.length + 1) : bundle, message: r.message });
    }
    return {
        success: true,
        installed,
...
export const handleSkills = async function (action, args, env) {
    const { ctx } = env;
    ...
    switch (action) {
        case "list_skills": {
            ...
            const seen = new Set(snapshot.skills.map((s) => s.name));
            ...
            for (const s of snapshot.skills) {
                ...
                if (!dirs.has(s.name) || (desc !== "" && !/^[>|][+-\d]*$/.test(desc)))
                    continue;
                ...
                if (parsed.description)
                    s.description = String(parsed.description);
            }
            return {
                skills: snapshot.skills.map((s) => {
                    ...
                    if (dirs.has(s.name))
                        info.managedFile = join(SKILLS_DIR, s.name, "SKILL.md");
                    return info;
                }),
            };
        }
        ...
        case "edit_skill": {
            const name = assertName(skillName, SKILL_NAME_RE, "skill");
            if (!description && action === "add_skill")
                return { error: "description is required for add_skill" };
            ...
            if (action === "edit_skill") {
                ...
                for (const [k, v] of Object.entries(parseSkillDoc(raw))) {
                    if (k !== "name" && k !== "description" && k !== "whenToUse")
                        extra[k] = v;
                }
            }
        ...
        case "install_skill": {
            ...
            if (!src)
                return { error: "sourcePath is required (folder containing SKILL.md)" };
            ...
            if (r.status === "exists")
                return { error: `Skill "${r.name}" already exists; pass overwrite:true to replace it` };
            if (r.status !== "ok")
        ...
        case "install_skill_files": {
            ...
            const hasManifest = files.some((f) => /(^|\/)SKILL\.md$/.test(String(f?.path ?? "")));
            if (!hasManifest)
                return { error: "files must include a SKILL.md" };
            ...
            try {
                ...
                for (const f of files) {
                    ...
                    if (total > 5 * 1024 * 1024)
                        return { error: "Bundle exceeds the 5 MB upload cap" };
