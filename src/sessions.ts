...
export const SESSIONS_ACTIONS = ["list_sessions", "delete_sessions"] as const;
...
async function scanSessions(): Promise<Row[]> {
  const rows: Row[] = [];
  ...
  for (const workspace of workspaces) {
    ...
    for (const dir of dirs.filter((d) => d.startsWith("session-"))) {
      ...
      for (const f of await readdir(full).catch(() => [] as string[])) {
        ...
        if (fst?.isFile()) {
          bytes += fst.size;
          if (fst.mtimeMs > modifiedMs) modifiedMs = fst.mtimeMs;
        }
      }
...
function tokenFor(rows: Row[]): string {
...
function liveIds(env: Env): Set<string> {
  const out = new Set<string>();
  try {
    const store = (env.ctx as any)?.sessions?.store;
    if (store instanceof Map) for (const id of store.keys()) out.add(String(id));
  } catch {}
  return out;
}

async function planDelete(args: Record<string, any>, env: Env): Promise<{ rows: Row[]; errors: string[] }> {
...
export const handleSessions: Handler = async function (action, args, env) {
  switch (action) {
    case "list_sessions": {
      const rows = await scanSessions();
      const live = liveIds(env);
    ...
    case "delete_sessions": {
      ...
      if (errors.length && !rows.length) return { error: errors.join("; ") };
