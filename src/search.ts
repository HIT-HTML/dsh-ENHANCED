...
export const SEARCH_ACTIONS = ["list_search", "set_search"] as const;

const KEY_FIELDS = ["exaApiKey", "tavilyApiKey", "keenableApiKey", "perplexityApiKey", "deepseekApiKey"] as const;

/** Coerce a Bing market code to the canonical lang-COUNTRY shape; passthrough otherwise. */
function normalizeMarket(v: string): string {
...
function parseConfig(text: string): Record<string, any> | null {
	const inner = splitInner(text, SEARCH_BEGIN, SEARCH_END);
	if (!inner.trim()) return null;
	const parsed = yaml.load(inner);
	const arr = Array.isArray(parsed) ? (parsed as Record<string, any>[]) : [];
	const entry = arr.find((r) => r && r.id === "web-search-free");
	return (entry && entry.config) || null;
}

async function writeSearch(profile: string, cfg: Record<string, any>): Promise<void> {
...
export async function readSearchConfig(profile: string): Promise<Record<string, any>> {
 ...
 */
export async function probeInstances(urls: string[]): Promise<Array<Record<string, string>>> {
	return Promise.all(
		urls.map(async (url) => {
			try {
...
export const handleSearch: Handler = async function (action, args, env: Env) {
	const { profiles } = env;
	switch (action) {
		case "list_search": {
			const cfg = parseConfig(await readPatch(profiles[0]));
			const hasKey: Record<string, boolean> = {};
			for (const k of KEY_FIELDS) hasKey[k] = !!cfg?.[k];
			const instances: string[] = Array.isArray(cfg?.searxngInstances) ? cfg.searxngInstances : [];
			return {
		...
		case "set_search": {
			...
			for (const profile of targets) {
				...
				for (const k of KEY_FIELDS) {
					...
					else if (existing[k] != null) cfg[k] = existing[k];
				}
...
export function registerSearchTool(ctx: any): void {
	const unregister = ctx.tools.register({
		name: "dsh_search",
		description:
			"Thin web-search passthrough: delegates to the active search provider (e.g. Free Search) registered " +
