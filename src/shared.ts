...
export interface Env {
  ...
  loaderRef?: { entries?: () => Iterable<{ id?: unknown; name?: unknown; fiber?: unknown }> };
...
export function assertName(name: unknown, pattern: RegExp, what: string): string {
...
export function splitBlock(text: string, begin: string, end: string): { head: string; tail: string } {
  const b = text.indexOf(begin);
  if (b < 0) return { head: text, tail: "" };
  const e = text.indexOf(end, b);
  if (e < 0) return { head: text.slice(0, b), tail: "" };
  let tailStart = e + end.length;
  if (text[tailStart] === "\r") tailStart++;
  if (text[tailStart] === "\n") tailStart++;
