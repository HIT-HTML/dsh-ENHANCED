...
export const COMPACT_ACTIONS = ["compact_status", "set_compact"] as const;

/** The staged thresholdRatio from the managed block, or null when absent. */
function compactOverride(text: string): number | null {
...
async function writeCompact(profile: string, ratio: number): Promise<void> {
...
export const handleCompact: Handler = async function (action, args, env: Env) {
  const { profiles } = env;
  switch (action) {
    case "compact_status": {
    ...
    case "set_compact": {
      ...
      if (!Number.isFinite(r)) return { error: "thresholdRatio must be a number" };
      ...
      for (const profile of targets) await writeCompact(profile, r);
