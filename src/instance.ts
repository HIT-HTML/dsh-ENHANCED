...
function detectedSupervisor(): string | null {
...
export function servingPort(argv: string[] = process.argv): number | null {
...
function respawnInvocation(launch: Launch): { file: string; args: string[]; shell: boolean; detached: boolean } {
...
function restartHelperSource(spawned: ReturnType<typeof respawnInvocation>, cwd: string, logs: { out: string; err: string }, port: number | null): string {
  return [
...
function scheduleSelfExit(ms: number): void {
  setTimeout(() => process.kill(process.pid, "SIGTERM"), ms);
}

export const INSTANCE_ACTIONS = ["shutdown_instance", "restart_instance"] as const;

export const handleInstance: Handler = async (action, _args, env: Env) => {
  ...
  if (action === "restart_instance") {
    ...
    const base = dshArgv();
    if (!base.cwd) return { error: "could not determine how this instance was launched (no bin entry in argv)" };
    const launch: Launch = { ...base, args: [...base.args, ...process.argv.slice(2)] };
