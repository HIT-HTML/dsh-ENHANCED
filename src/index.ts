...
const ACTIONS = [...SKILL_ACTIONS, ...MCP_ACTIONS, ...COMPACT_ACTIONS, ...INSTANCE_ACTIONS, ...PLUGIN_ACTIONS, ...SESSIONS_ACTIONS, ...SEARCH_ACTIONS, ...FETCH_ACTIONS] as const;

/** Feature modules in dispatch order. */
const HANDLERS: Handler[] = [handleSkills, handleMcp, handleCompact, handleInstance, handlePlugins, handleSessions, handleSearch, handleFetch];
...
export function apply(ctx: any, config?: { mcpProfiles?: string[]; allowRestart?: boolean }) {
  ...
  async function performAction(raw: unknown): Promise<Record<string, unknown>> {
    ...
    try {
      const env: Env = {
        ...
        targets() {
          const requested = args.profiles as string[] | undefined;
  ...
  const unregister = ctx.tools.register({
    ...
    parameters: {
      ...
      properties: {
        ...
        mcpConfig: {
          type: "object",
          description:
            "Connection config for add_mcp: stdio needs {command, args?, env?}; " +
    ...
    execute: (args: any) => performAction(args),
  });
  ...
  const searchConfig: Record<string, any> = { provider: "bing" };
  ...
  if (connection?.rpc?.handle) {
    connection.rpc.handle(
      "/dsh-enhanced",
      async (_endpoint: string, payload: unknown) => ({ ok: true, value: await performAction(payload) }),
  ...
  if (typeof ctx.inject === "function") {
    ctx.inject(["settings"], (scope: any) => {
      try {
        const passThrough = (value: unknown) => ({ ...((value as object) ?? {}) });
        (passThrough as any).toJSON = () => ({ uid: 0, refs: { 0: { type: "object", meta: { default: {} }, dict: {} } } });
