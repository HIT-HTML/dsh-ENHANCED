 ...
 */
import type { Handler, Env } from "./shared.js";

export const FETCH_ACTIONS = ["web_fetch"] as const;
...
async function fetchOnce(url: string, depth = 0): Promise<{ status: number; finalUrl: string; contentType: string; body: string }> {
	...
	if (status >= 300 && status < 400) {
		if (depth >= 5) throw new Error(`too many redirects (${depth} hops)`);
		const loc = res.headers.get("location");
		if (loc) return fetchOnce(new URL(loc, url).toString(), depth + 1);
	...
	if (!reader) return { status, finalUrl, contentType, body: "" };
...
function renderBody(contentType: string, body: string): string {
	const isHtml = /text\/html|application\/xhtml/i.test(contentType);
 ...
 */
export function registerFetchTool(ctx: any): void {
	const unregister = ctx.tools.register({
		name: "web_fetch",
		description:
			"Retrieves a specific HTTP(S) URL. HTML bodies are rendered to markdown " +
 ...
 */
export const handleFetch: Handler = async function () {
	return {};
};